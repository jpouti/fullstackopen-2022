...
test('renders only blog title and author by default view, 5.13 STEP 1', () => {
  ...
  const { container } = render (<Blog blog={blog} handleLike={mockHandler} handleDeleteBlog={mockHandler} />)
...
test('When like button is pressed twice, event handler is called twice, 5.15 STEP 3', async () => {
  ...
  render (
    <Blog blog={blog} handleLike={mockHandler} handleDeleteBlog={mockHandlerDelete} />
  )
