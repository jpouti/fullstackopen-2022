...
const favoriteBlog = (testBlogs) => {
  const favorite = testBlogs.reduce((accumulator, likes) =>
    (accumulator.likes > likes.likes ? accumulator : likes), 0)
  return { title: favorite.title, author: favorite.author, likes: favorite.likes }
}

const mostBlogs = (testBlogs) => {
  let countBlogs = _.map(
    _.countBy(testBlogs, 'author'), (count, author) =>
...
const mostLikes = (testBlogs) => {
  ...
  let max = Object.entries(blogLikes).reduce((max, entry) => (entry[1] >= max[1] ? entry : max))
